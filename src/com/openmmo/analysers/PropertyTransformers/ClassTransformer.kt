...
class ClassTransformer : IdentityMapper.Class() {
    override val predicate = predicateOf<ClassWrapper> { it.instanceMethods.isNotEmpty() }
        ...
        .and { it.instanceMethods.any { it.arguments.size == 2 &&
                ...
                it.invokesMethod(Opcodes.INVOKESTATIC, "forName", "java/lang/Class") } }

    @DependsOn(ClassTransformer::class)
    class instance : IdentityMapper.StaticField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<ClassTransformer>() }
    }
