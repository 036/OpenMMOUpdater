...
class DoubleTransformer : IdentityMapper.Class() {
    override val predicate = predicateOf<ClassWrapper> { it.instanceMethods.isNotEmpty() }
        ...
        .and { it.instanceMethods.any { it.arguments.size == 2 &&
                ...
                it.invokesMethod(Opcodes.INVOKESTATIC, "parseDouble", "java/lang/Double") } }

    @DependsOn(DoubleTransformer::class)
    class instance : IdentityMapper.StaticField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<DoubleTransformer>() }
    }
