...
class FloatTransformer : IdentityMapper.Class() {
    override val predicate = predicateOf<ClassWrapper> { it.instanceMethods.isNotEmpty() }
        ...
        .and { it.instanceMethods.any { it.arguments.size == 2 &&
                ...
                it.invokesMethod(Opcodes.INVOKESTATIC, "parseFloat", "java/lang/Float") } }

    @DependsOn(FloatTransformer::class)
    class instance : IdentityMapper.StaticField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<FloatTransformer>() }
    }
