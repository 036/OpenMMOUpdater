...
class InetAddressTransformer : IdentityMapper.Class() {
    override val predicate = predicateOf<ClassWrapper> { it.instanceMethods.isNotEmpty() }
        ...
        .and { it.instanceMethods.any { it.arguments.size == 2 &&
                ...
                it.invokesMethod(Opcodes.INVOKESPECIAL, "<init>", "java/net/InetSocketAddress") } }

    @DependsOn(InetAddressTransformer::class)
    class instance : IdentityMapper.StaticField() {
        override val predicate = predicateOf<FieldWrapper> { it.type == type<InetAddressTransformer>() }
    }
