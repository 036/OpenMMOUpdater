package com.openmmo.deobfuscator.Common
...
fun writeClasses(classes: Iterable<ByteArray>, jar: Path) {
    val unique = classes.distinctBy { ClassReader(it).className }
    ZipUtil.pack(unique.map { ByteSource("${ClassReader(it).className}.class", it) }.toTypedArray(), jar.toFile())
}
